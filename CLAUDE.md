...
src/
  ...
  lexer/                    # Phase 1: 렉서 (토크나이저)
    mod.zig                 #   렉서 엔트리 + re-export
    token.zig               #   토큰 종류(Kind), Span, Token, 키워드 맵
    scanner.zig             #   스캔 로직 (구현 예정)
    unicode.zig             #   유니코드 식별자 처리 (구현 예정)
    simd.zig                #   SIMD 최적화 (구현 예정)
    string.zig              #   문자열 리터럴 파싱 (구현 예정)
    number.zig              #   숫자 리터럴 파싱 (구현 예정)
