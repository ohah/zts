...
pub fn parseObjectProperty(self: *Parser) ParseError2!NodeIndex {
    ...
    if (self.current() == .kw_get or self.current() == .kw_set) {
        ...
        if (peek != .colon and peek != .l_paren and peek != .comma and peek != .r_curly) {
            ...
            const key = try parsePropertyKey(self);
    ...
    if (self.eat(.colon)) {
        value = try parseAssignmentExpression(self);
    } else if (self.eat(.eq)) {
...
pub fn parseObjectMethodBody(self: *Parser, start: u32, key: NodeIndex, flags: u16) ParseError2!NodeIndex {
    ...
    while (self.current() != .r_paren and self.current() != .eof) {
        const param = try parseBindingIdentifier(self);
...
pub fn tryWrapDefaultValue(self: *Parser, node: NodeIndex) ParseError2!NodeIndex {
    if (self.eat(.eq)) {
        ...
        const default_val = try parseAssignmentExpression(self);
