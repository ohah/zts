...
| # | 항목 | 비고 |
|---|------|------|
| 57 | 제네릭 함수 Arena 테스트 signal 9 | 테스트 러너에서 프로세스 kill됨. CLI는 정상 동작. 원인 미파악 (Zig 버전업 후 재시도) |
| 58 | Zig 0.15 마이그레이션 | `std.io.getStdOut()` → `std.fs.File.stdout()` + buffered writer 등 breaking change |
